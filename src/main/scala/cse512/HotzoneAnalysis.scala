...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = 
  {
    ...
    val ansDF = spark.sql("select rectangle,count(point) as number_of_points from joinResult group by rectangle order by rectangle").persist()
    ansDF.createOrReplaceTempView("Answer_View")
    return ansDF.repartition(1)
